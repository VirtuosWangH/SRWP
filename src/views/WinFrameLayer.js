...
//        this.setVisible(false);
        this.winFrames = [];
        cc.SpriteFrameCache.getInstance().addSpriteFrames(res.textureTablet_plist);
        this.createUI();
    },
    createUI:function(){
...
//        this.addChild(winNum);
    ...
    showWinFrame:function(lineIndex,symbolAry){
        ...
        for (var i=0; i<rowNum; i++) {
            ...
            for (var j=0; j<symbolNum; j++) {
                ...
                if(value){
                    ...
                    var winFrame = cc.Sprite.createWithSpriteFrameName(frameName);
                    var targetX = j*165;
                    var targetY = (3-i)*121;
                    if(firstFrameY == null){
                        firstFrameY = targetY;
        ...
        var lineNum = cc.Sprite.createWithSpriteFrameName(circleName);
        lineNum.setPosition(-70,firstFrameY);
        this.addChild(lineNum);

        var tabName = "wintab/winboxbg-0"+index+".png"
        var winTab = cc.Sprite.createWithSpriteFrameName(tabName);
